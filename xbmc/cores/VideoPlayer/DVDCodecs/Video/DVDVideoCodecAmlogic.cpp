...
// compile the payload latch section of AMLFrameMetadata.h into this unit
#define AML_FRAME_METADATA_LATCH
...
//      break;
      ...
      if (m_hints.extradata && !m_hints.cryptoSession)
      {
        if (aml_support_dolby_vision())
        {
                  ...
                  CSettings::SETTING_COREELEC_AMLOGIC_DV_LED) == AML_DV_TV_LED)
          {
            ...
            if (zeroLevel5)
              m_streamMeta.doviFlags = "l5-zeroed";
          }
               ...
               aml_get_cpufamily_id() == AML_S5)
          {
            ...
            if (!m_streamMeta.doviFlags.empty())
              m_streamMeta.doviFlags += ' ';
            m_streamMeta.doviFlags += "converted";
          }
        }
      }
  ...
  if (m_bitstream)
  {
    ...
    if (!caps.SupportsHDR10Plus())
    {
      ...
      if (!m_streamMeta.doviFlags.empty())
        m_streamMeta.doviFlags += ' ';
      m_streamMeta.doviFlags += "hdr10plus-removed";
    }
        ...
        m_hints.dovi.dv_profile != 5)
    {
      ...
      if (!m_streamMeta.doviFlags.empty())
        m_streamMeta.doviFlags += ' ';
      m_streamMeta.doviFlags += "rpu-removed";
    }
  }
  ...
  // config record from hints, not m_hints, which the P7 to P8 conversion
  // above has already rewritten
...
bool CDVDVideoCodecAmlogic::AddData(const DemuxPacket &packet)
{
  ...
  if (pData)
  {
    ...
    if (m_hints.codec == AV_CODEC_ID_HEVC && !packet.isDualStream)
    {
      ...
      AMLLatchAv1DoviRpu(pData, iSize, m_pendingMeta);
  ...
  if (packet.pSideData && packet.iSideDataElems > 0)
  {
    ...
    {
      // mkv block additions carry the payload here instead of the SEI

...
#if defined(AML_FRAME_METADATA_LATCH)
extern "C"
{
...
// published base64 encoded under the player.process(video.sidedata) label. The
// latch section below is compiled only into DVDVideoCodecAmlogic.cpp.

struct AMLFrameMetadata
{
  std::string doviConfig;
  std::string doviFlags;
  std::string doviRpu;
  ...
  std::string ComposeSideData() const
  {
    const std::pair<const char*, const std::string*> entries[] = {
        {"dovi.config", &doviConfig}, {"flags", &doviFlags},  {"dovi.rpu", &doviRpu},
        {"hdr10plus", &hdr10pSei},    {"mdcv", &hdrMdcv},     {"cll", &hdrCll}};
    std::string json;
...
{
  if (static_cast<uint32_t>(info) != CE_PLAYER_PROCESS_VIDEO_SIDEDATA)
    return false;
...
// tail of the T.35 header every HDR10+ payload starts with: country 0xB5,
...
// the payload layout shared by the HEVC SEI and the MKV block addition side data
...
// HDR10+ T.35, the mastering display colour volume (137) and the content
// light level (144), each latched verbatim after unescaping.
...
// Scans AV1 OBUs for the Dolby Vision T.35 metadata OBU (provider 0x003B) and
// latches the payload from the country code on, so an add-on can tell it from
// an HEVC RPU by its first byte.
inline void AMLLatchAv1DoviRpu(const uint8_t* data, size_t size, AMLFrameMetadata& meta)
{
  if (!data)
    return;
...
#endif // AML_FRAME_METADATA_LATCH
